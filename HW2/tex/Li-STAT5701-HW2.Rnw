...
The actual distribution of the random p-value is Unif$(0, 1)$. 
\proof
...
\end{aligned}
\end{equation*}
i.e., $\forall \alpha \in [0, 1]$ $\mathrm{P}(p < \alpha) = \alpha$, which implies that p-value has a uniform distribution under $H_{0}$. Q.E.D.
...
The sequence of values for $\mu_2$ we select is $68, 69, 70, 71$, and the corresponding power for these values are $0.050, 0.295, 0.855, 0.999$.
